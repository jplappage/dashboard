...
**Cinema-date-reached rule (flip to VOD, verify availability first):** the moment a film's cinema date is **today or earlier** (i.e. it has opened), it is no longer pre-theatrical — do NOT leave it showing `cinema: true` with the cinema date as `vodDate` (that renders "🎬 Cinemas …" and no VOD estimate, which JP flagged). Remove `cinema: true` and queue it for an IMDb check now that it's released. Then, for the `vodDate`, **check whether it is ALREADY buyable digitally before assuming a ~45-day window** — many mid-budget titles (Lionsgate, Sony genre, indie/A24 SXSW pickups) go **day-and-date PVOD**, i.e. buyable on the theatrical date. WhenToStream lags on these (shows "PVOD TBA"), so check JustWatch (`justwatch.com/us/movie/<slug>`) or search `"[title]" where to watch buy Fandango At Home`:
  - **Genuinely buyable/rentable now** → it's out: set `vodDate` to the day-and-date/theatrical date, `platform: 'Digital'`, `estimated: false`, remove the `note` (confirmed = no note). ⚠️ **A Fandango At Home / JustWatch / Plex "buy" listing is NOT proof of release** — these stores publish a purchase/pre-order page *before* a title is actually available, so a listing alone gives false positives. Only flip to confirmed if there's a real, current purchase/rental price (not "pre-order"/"coming soon") AND ideally a second corroborating source; when unsure, leave it on the estimate and flag it to JP rather than flipping. (Learned 23 Aug 2026: those listings falsely showed **Insidious** and **The Rivals of Amziah King** as buyable when they were still theatrical-only — only **Mutiny** was genuinely out. JP: "they are not available to buy or rent.")
  - **Genuinely theatrical-only** (no digital listing yet) → fall back to a Tuesday-snapped `estimated: true` date ~45 days after the cinema open (studio windows: WB PVOD ~31 days, A24 digital ~45–60 days) with a `note` describing the window.
Only future-cinema films keep `cinema: true`. **Also re-check released films that are still on a `~45-day` estimate each refresh** — but per the ⚠️ above, only promote to confirmed when a purchase/rental is genuinely live (verified price, not a placeholder listing), not on the strength of a store page alone.
...
- Any `next` field with a specific date that has now passed → increment `aired`, update `next` to null or next known info, AND set `airing:"S<N>"` (the season number is the only part the hub page reads — a human-readable suffix like `· Wednesdays` is fine but ignored). Set `airingUntil:"YYYY-MM-DD"` to the finale date **only for weekly shows**, since show-log.html still uses it to place a rolling season in *Out Now*; **all-at-once drops get no `airingUntil`** — there is no rollout to track. **Use the UK viewing day** for any human-readable label — US Sunday-night shows land Monday in the UK.
- **Do NOT strip `airing` when `airingUntil` passes** (rule changed 8 Sep 2026). The hub page's Out-now list runs until JP ticks that season watched in the show log, not until a date — an unwatched season that finished airing should stay on the hub. Clear `airing`/`airingUntil` only when the gist's `showLog` has that season marked `w` (watched) or `a` (abandoned), or when a newer season supersedes it. Read the marks from the gist during Phase 1 if you need to check:
  ```js
  (await (await fetch('https://api.github.com/gists/5411ac8d12fdc31aa3fa73e4d66f6377',{cache:'no-store'})).json())
  ```
  → `JSON.parse(files['watchlist.json'].content).showLog['<Show Name>']` → `{s1:'w', s2:'w'}`.
- **Never invent an airing window.** Episode counts, how many have aired and the weekly release day are read live from TVmaze by the hub page (`index.html`, cached 6h in localStorage `hub-eps-v1`), keyed off each show's `imdbId`. Nothing about episode numbers belongs in `shows-data.js`. If a show's hub line shows no episode detail, its `imdbId` is wrong or TVmaze has no episode list yet — fix the id rather than hand-writing counts.
...
**1-month reliability rule (was 3 months; JP tightened it 5 Sep 2026):** A premiere/VOD date more than **1 month** in the future is not dependable yet — treat it as an estimate only, even if officially announced. For shows: still store the real date in `next` (e.g. `"S3 · 11 Nov 2026"`); the show-log page automatically keeps it in *Awaiting Date* until it falls within the 1-month window, then promotes it to *Date Announced*. For films: don't set a firm `vodDate` for a date >1 month out — keep `estimated: true` with a `note` describing the window. Dated shows beyond the window MUST still get the dated-show sanity check each refresh (Phase 2).
